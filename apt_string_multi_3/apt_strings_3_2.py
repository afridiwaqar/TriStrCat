...
# Plot individual ROC curves with different line styles
...
for result in results:
    if result['name'] != 'Neural Network':  # Skip Neural Network as it uses a different predict_proba method
        plt.figure(figsize=(8, 6))
        for i, (class_name, line_style) in enumerate(zip(class_names, line_styles)):
            fpr, tpr, _ = roc_curve(y_test == i, result['y_pred_proba'][:, i])
            roc_auc = auc(fpr, tpr)
            plt.plot(fpr, tpr, linestyle=line_style, lw=2, label=f'{class_name} (AUC = {roc_auc:.2f})')
